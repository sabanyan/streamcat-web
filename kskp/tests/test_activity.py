...
class ActivityTest(unittest.TestCase):
    ...
    def test_make_finished_history(self):
        ...
        result = {'o_section':frame_o_section, 'o_all':frame_o_all}
        ...
        # 実行履歴ファイルの更新
        make_finished_histroy(flow_uuid, file_path, result)

        # 更新後の中身をテスト
        executed_data = json.loads(file_path.read_text(encoding='utf-8'))
        self.assertEqual([data for data in executed_data['data'].keys()], [key for key in result.keys()])
