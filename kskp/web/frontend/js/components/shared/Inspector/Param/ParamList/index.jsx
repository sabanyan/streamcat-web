...
export default class ParamList extends React.Component<Props, State> {
        ...
        try {
            switch (param.type) {
                ...
                case Constants.param.type.string  :
                    paramElement = <ParamString label={label} param={param} disabled={disabled} value={value}
                                                onChange={onChange} />;
                    break;
                case Constants.param.type.boolean :
                    paramElement = <ParamBoolean label={label} param={param} disabled={disabled} value={value}
                                                 onChange={onChange} />;
                    break;
                case Constants.param.type.select  :
                    paramElement = <ParamSelect label={label} param={param} disabled={disabled} value={value}
                                                onChange={onChange} />;
                    break;
    ...
    renderElement(param: CommandParamType, argIndex: number, arg: Array<CommandParamType>): JSX.Element {
        ...
        param.elements.forEach((element: CommandParamType, index: number) => {
            ...
            ele = <div key={argIndex + element.name + index} className={className}>
                {this.getParamElement(element, false, undefined, value, (e, param, elementValue) => {
                    this.onChangeContent(e, element, elementValue, argIndex);
                }, headers)}
            </div>;
            elements.push(ele);
        });

        const deleteButton = (this.state.deletable) ? <Button danger={true} onClick={(e) => {
            this.onDeleteElement(e, param, argIndex);
        }}>削除</Button> : null;
    ...
    renderElements(param: CommandParamType, arg: Array<CommandParamType>) {
        ...
        let contents = (this.state.draggable) ? <SortableList distance={1} items={paramElements}
                                                              onSortEnd={(value, e) => this.onSortEnd(value, e)} /> : paramElements;
        return <React.Fragment>
