...
const FlowInspector = (props: Props) => {
    ...
    const nullInspector = () => {
        return <Resizer>
            <BaseInspector key={"null"} disabled={false}/>
        </Resizer>;
    };
    ...
    const content = <div>
        ...
        <div>
            {moment(createdAt, 'YYYY-MM-DD hh:mm:ss', false).format(Constants.format.dateTime)}
        </div>
    </div>;

    return <Resizer>
        <BaseInspector key={uuid + "_" + label} label={label} onBlurTitle={(e) => onBlurTitle(e, flow)} disabled={false}>
            {content}
        </BaseInspector>
    </Resizer>;
