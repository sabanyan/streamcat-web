...
export default class ParamUtil {
  ...
  static getArgValue(currentTarget: any): any {
  ...
  static getAllSelectedValue(element) {
    if (element.multiple) {
      return Array.from(element.selectedOptions).map((v: any) => v.value)
    } else {
  ...
  static getParamElement(param, onBuild, events, defaultValue, refValue, headers) {
    ...
    switch (param.type) {
      case Constants.param.type.number:
        paramElement = <ParamNumber 
          param={ param } defaultValue = { defaultValue }
