...
class VisConverter:
    ...
    def __init__(self, vis):
        if isinstance(vis, BokehPlotVis):
            result = vis.result
            self.response = render_template('visualize/component.html', script=result['script'], div=result['div'])
        elif isinstance(vis, Vis):
            result = vis.result
            self.response = render_template('visualize/table.html', header=result['header'], reader=result['reader'])
        else:
