...
@jsonify
@update_users_info
async def get_users(q:str=None, except_inactive=False, roles=False, projects=False, finder:Finder=Depends(get_finder)):
    """
    ...
    if search_keyword is None:
        return finder.user.find_all(except_states=states)
    else:
        return finder.user.find_by_keyword(search_keyword, except_states=states)

@router.get('/users/self')
@login_required_api
@jsonify
@update_user_info
async def get_self(except_inactive=False, roles=False, projects=False, finder:Finder=Depends(get_finder)):
    """
    自身のユーザを取得する
    """
    states = _get_except_states(except_inactive)
    return finder.user.find_by_id(finder.myself.id, except_states=states)

@router.get('/users/{user_uuid}')
@login_required_api
@jsonify
@update_user_info
async def get_user(user_uuid, except_inactive=False, roles=False, projects=False, finder:Finder=Depends(get_finder)):
    """
    指定したユーザを取得する
    """
    states = _get_except_states(except_inactive)
    return finder.user.find_by_uuid(user_uuid, except_states=states)

@router.post('/users')
@login_required_api
@jsonify
async def make_new_user(request:Request, finder:Finder=Depends(get_finder)):
    """
    新しいユーザを作成する
    """
    ...
    new_user = finder.user.create(email=req['email'], name=req['name'], password=req.get('password'))
    new_user.save()
    return new_user
...
@login_required_api
@jsonify
async def update_self(request:Request, finder:Finder=Depends(get_finder)):
    """
    自身のユーザを変更する
    """
    req = RequestJson(await request.json())
    user = finder.user.find_by_id(finder.myself.id)
...
@login_required_api
@jsonify
async def update_user(request:Request, user_uuid, finder:Finder=Depends(get_finder)):
    """
    指定したユーザを変更する
    'password':Noneの場合はパスワードを自動生成する
    """
    req = RequestJson(await request.json())
    user = finder.user.find_by_uuid(user_uuid)
    return _update_user_inner(user, req)
...
@login_required_api
@jsonify
async def delete_user(user_uuid, finder:Finder=Depends(get_finder)):
    """
    指定した登録状態のユーザを論理削除する
    (仮登録ユーザは物理削除する)
    """
...
@jsonify
@update_roles_info
async def get_roles(members=False, finder:Finder=Depends(get_finder)):
    """
    全てのロールを取得する
    """
    return finder.role.find_all()

@router.get('/roles/{role_uuid}')
@login_required_api
@jsonify
@update_role_info
async def get_role(role_uuid, members=False, finder:Finder=Depends(get_finder)):
    """
    指定したロールを取得する
    """
    return finder.role.find_by_uuid(role_uuid)

@router.post('/roles')
@login_required_api
@jsonify
async def make_new_role(request:Request, finder:Finder=Depends(get_finder)):
    """
    新しいロールを作成する
    """
    req = RequestJson(await request.json())
    ...
    new_role = finder.role.create(name=req['name'])
    new_role.save()
    return new_role
...
@login_required_api
@jsonify
async def update_role(request:Request, role_uuid, finder:Finder=Depends(get_finder)):
    """
    指定したロールを変更する
    """
    ...
    role = finder.role.find_by_uuid(role_uuid)
...
@login_required_api
@jsonify
async def delete_role(role_uuid, finder:Finder=Depends(get_finder)):
    """
    指定したロールを削除する
    """
    role = finder.role.find_by_uuid(role_uuid)
...
@login_required_api
@jsonify
async def join_user_to_sys_admin_role(user_uuid, finder:Finder=Depends(get_finder)):
    """
    システム管理者ロールにユーザを追加する
    """
    # システム管理者による、システム管理者の追加・削除は不可なので、owner=Falseでシステム管理者ロールに追加する
    sys_admin_role = finder.role.load_sys_admin_role()
    _join_user_to_role(finder, sys_admin_role.uuid, user_uuid, owner=False)

@router.put('/roles/usr_admin/users/{user_uuid}')
@login_required_api
@jsonify
async def join_user_to_usr_admin_role(user_uuid, finder:Finder=Depends(get_finder)):
    """
    ユーザ管理者ロールにユーザを追加する
    """
    # ユーザ管理者による、ユーザ管理者の追加・削除を可能とするため、owner=Trueでユーザ管理者ロールに追加する
    usr_admin_role = finder.role.load_usr_admin_role()
    _join_user_to_role(finder, usr_admin_role.uuid, user_uuid, owner=True)

@router.put('/roles/{role_uuid}/users/{user_uuid}')
@login_required_api
@jsonify
async def join_user_to_role(request:Request, role_uuid, user_uuid, finder:Finder=Depends(get_finder)):
    """
    ロールにユーザを追加する
    """
    req = RequestJson(await request.json())
    ...
    _join_user_to_role(finder, role_uuid, user_uuid, req['owner'])

def _join_user_to_role(finder:Finder, role_uuid, user_uuid, owner:bool):
    from streamcat.store.auth import Role, NoRoleOwnerException
...
@login_required_api
@jsonify
async def leave_user_outof_sys_admin_role(user_uuid, finder:Finder=Depends(get_finder)):
    """
    システム管理者ロールからユーザを削除する
    """
    from streamcat.store.auth import NoRoleOwnerException
    sys_admin_role = finder.role.load_sys_admin_role()
    _leave_user_outof_role(finder, sys_admin_role.uuid, user_uuid, raise_on_no_owner=False)

@router.delete('/roles/usr_admin/users/{user_uuid}')
@login_required_api
@jsonify
async def leave_user_outof_usr_admin_role(user_uuid, finder:Finder=Depends(get_finder)):
    """
    ユーザ管理者ロールからユーザを削除する
    """
    from streamcat.store.auth import NoRoleOwnerException
    usr_admin_role = finder.role.load_usr_admin_role()
    try:
        _leave_user_outof_role(finder, usr_admin_role.uuid, user_uuid)
...
@login_required_api
@jsonify
async def leave_user_outof_role(role_uuid, user_uuid, finder:Finder=Depends(get_finder)):
    """
    ロールからユーザを削除する
    """
    _leave_user_outof_role(finder, role_uuid, user_uuid)

def _leave_user_outof_role(finder:Finder, role_uuid, user_uuid, raise_on_no_owner=True):
    from streamcat.store.auth import NoRoleOwnerException
...
@login_required_api
@jsonify
async def join_user_to_project(request:Request, project_uuid, user_uuid, finder:Finder=Depends(get_finder)):
    """
    プロジェクトにユーザを追加する
    """
    ...
    project = finder.data.find_by_uuid(project_uuid, type=SavableDatum.PROJECT_TYPE)
    user = finder.user.find_by_uuid(user_uuid)
...
@login_required_api
@jsonify
async def leave_user_outof_project(project_uuid, user_uuid, finder:Finder=Depends(get_finder)):
    """
    プロジェクトからユーザを削除する
    """
    from streamcat.core import SavableDatum
