...
export const useMyDragLayer = <TItem,>() => {
    const { items, isDragging, initialOffset, differenceOffset } = useDragLayer(monitor => ({
        ...
        items: monitor.getItem() as TItem[],
        isDragging: monitor.isDragging(),
    }));

    if (!isDragging || !differenceOffset || !initialOffset) {
        return { items: [], isDragging: isDragging, x: 0, y: 0 };
    }
