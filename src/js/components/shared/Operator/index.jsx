...
export default class Operator extends React.Component{
  ...
  buildArgumentsContent(){
    ...
    const argument_inputs = this.props.arguments.map((argument)=>{
      return <div key={self.props.name + "_" + argument.name} className="mb-8px">
        ...
        <input type="text" className="form-control" placeholder={argument.name} ref={(element)=>{if(element)(self.inputRefs.push({argument:argument,element:element}))}} defaultValue={""}></input>
        </div>
    })
  ...
  onClickOperator(){
    ...
    ModalUtil.registerModal({
      id: Constants.modal.ADD_OPERATOR, onClickDone: () => {
        ...
        //オペレータを追加
        const add_step = new OperatorModel({operator:self.props.name,text:this.props.description,parameters:parameters})
        ...
        const output_steps = self.props.outputs.map((output_step)=>{
          ...
          return new DataSourceModel({type:output_step.type,operator:"mtee",text:self.props.description + "の結果",property:{hasData:false},parameters:{"o": "/kskp/data/frame/"+ self.props.name +".csv"}})
        })
  ...
  render () {
    return <div>
      <div className="operator no-icon" onClick={()=>this.onClickOperator()}>
